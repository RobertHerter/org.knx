...
class GenericKNXDevice extends Homey.Device {
  ...
  onInit() {
    ...
    // Use the global active gateway — one gateway for all devices.
    // Fall back to per-device MAC lookup for backward compatibility.
    const activeInterface = this.knxInterfaceManager.getActiveInterface()
      || this.knxInterfaceManager.getKNXInterface(this.macAddress);

    if (activeInterface) {
      this.setKNXInterface(activeInterface);
    } else {
      this.log('no interfaces found, waiting');
  ...
  /**
   * Handler for the interface found — accept any interface since there is
   * exactly one global gateway shared by all devices.
   */
  onKNXInterface(knxInterface) {
    if (knxInterface && !this.knxInterface) {
